...
export function registerActionTools(server, client) {
    server.tool("list_actions", "List affiliate conversions/actions with filtering by state, date range, campaign, and media partner", {
        state: z
        ...
        const data = await client.request("/Actions", {
            params: {
                State: params.state,
    ...
    server.tool("get_action", "Get details for a specific action/conversion by ID", {
        actionId: z.string().describe("The action ID to retrieve"),
    }, async ({ actionId }) => {
        const data = await client.request(`/Actions/${actionId}`);
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("update_action", "Update an action - approve, adjust amount, or change state. Reason codes: APPROVED, CONS_FRAUD, ITEM_RETURNED, ORDER_UPDATE, REJECTED, TEST_ACTION", {
        ...
        const data = await client.request(`/Actions/${params.actionId}`, { method: "PUT", body });
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("get_action_items", "List line items within a specific action (order-level detail)", {
        actionId: z.string().describe("The action ID"),
    }, async ({ actionId }) => {
        const data = await client.request(`/Actions/${actionId}/Items`);
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("list_action_updates", "List history of action updates/modifications", {
        startDate: z.string().optional().describe("Start date (YYYY-MM-DD)"),
        endDate: z.string().optional().describe("End date (YYYY-MM-DD)"),
    }, async (params) => {
        const data = await client.request("/ActionUpdates", {
            params: {
